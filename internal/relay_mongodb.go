...
func (r *mongoRelay) parseHandshake(ctx context.Context, conn net.Conn) (string, string, map[string]string, error) {
	for {
		...
		doc, requestID, err := ReceiveMongoMessage(conn)
		if err != nil {
		...
		if isHandshakeMessage {
			...
			builder := bsoncore.NewDocumentBuilder().
				...
				AppendDouble("ok", 1.0).
				AppendArray("saslSupportedMechs", bsoncore.NewArrayBuilder().
					AppendString("SCRAM-SHA-256"). // No SCRAM-SHA-1 support since SHA-256 is more secure and widely supported
					Build(),
				)
			...
			responseMsg = append(responseMsg, builder.Build()...)
...
func (r *mongoRelay) createSessionUser(ctx context.Context) error {
	...
	resp, err := r.secretsEngine.NewUser(ctx, dbplugin.NewUserRequest{
		...
		Statements: dbplugin.Statements{
			Commands: []string{fmt.Sprintf(`{"db": %q}`, r.sessionDatabase)},
		},
	})
	if err != nil {
